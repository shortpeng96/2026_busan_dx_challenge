...
| :--- | :--- | :--- |
| **ROC-AUC** | **0.946** | 5-fold CV, 하이브리드 모델 |
| **RMSLE** | **1.281** | 기하급수적 농도 증가(Log)를 고려한 오차율 |
| **R² (설명력)** | **0.507** | 수질 변동성 설명력 |
| **MAE** | **131.6** | 대장균 농도 절대 오차 (cfu/100ml) |
...
| **Phase 4 (최종 최적화)** | `Dual-Output Regressor` 아키텍처 및 이중 기준선 분리 | **0.946** | 최종 엔터프라이즈 레벨 성능 달성 |
...
> [!WARNING]
> ### 🟡 1단계: 주의 알림 (기준 점수: 0.562)
> * **목적**: 관리자에게 선제적 경고 알림 발송
> * **재현율 (Recall)**: **76.9%** (오염 사태 10/13건 선제 탐지)
> * **오탐 (False Positive)**: 6건

> [!CAUTION]
> ### 🔴 2단계: 입수 통제 (기준 점수: 1.655)
