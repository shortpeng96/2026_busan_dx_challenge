"""
03_generate_results.py - Dadaepo Beach Water Quality (Advanced)
"""
import pandas as pd
import numpy as np
...
ax.set_title(f'일광 해수욕장 - 수질 예측 핵심 변수 중요도', color=C_NAVY, weight='bold', fontsize=16)
...
# Add AUC score to the center of the donut
...
plt.suptitle(f'일광 해수욕장 이중 기준선 혼동행렬 (Confusion Matrix)', color=C_NAVY, weight='bold', fontsize=16)
plt.tight_layout()
...
# Use Seaborn's KDE plot with log_scale=True to handle massive right-skewed outliers properly
# Add a tiny epsilon (1e-5) to avoid log(0) issues if any score is exactly 0
epsilon = 1e-5
...
# Set Labels and Titles to be extremely clear
ax.set_title(f'일광 해수욕장 - 정상/오염 데이터별 예측 점수 분포도 (Log Scale)', color=C_NAVY, weight='bold', fontsize=16)
...
# Formatting X-axis to show regular numbers instead of scientific notation
...
# Baseline stats (Hardcoded from previous logic: TP=23 (50% recall of 46), FP=10)
...
# Plot 1: Recall Comparison (Baseline vs Warning)
...
plt.suptitle(f'일광 해수욕장 - AI 도입 효과 (ROI) 시각화', color=C_NAVY, weight='bold', fontsize=16)
plt.tight_layout()
...
### 2.1 분류기(Classifier) 다이렉트 타겟팅 및 듀얼 앙상블
- 오염/정상 여부를 수치로 먼저 예측하는 다대포/해운대 방식과 달리, 오염(1)/정상(0) 클래스 자체를 직접 타겟팅하는 XGBClassifier를 주력으로 사용합니다. 또한, 시계열 대시보드 시각화의 안정성을 위해 백그라운드에 보조 회귀기(XGBRegressor)를 결합한 듀얼 앙상블 구조를 채택했습니다.
