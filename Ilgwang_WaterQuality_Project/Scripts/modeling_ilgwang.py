...
best_feats = ['tide_range', 'discharge_95th_thresh', 'precip_3d_sum_lag', 'nps_first_flush']
...
if 'log_ecoli' not in df_master.columns:
    if 'ecoli_max' in df_master.columns:
        df_master['log_ecoli'] = np.log1p(df_master['ecoli_max'])
    elif 'ecoli' in df_master.columns:
...
print('3. Hyperparameter Tuning (GridSearchCV) for Regressor...')
param_grid = {
    'max_depth': [3, 4, 5],
...
grid = GridSearchCV(xgb_reg, param_grid, cv=KFold(n_splits=3, shuffle=True, random_state=42), scoring='neg_mean_squared_error', n_jobs=-1)
grid.fit(X_imp, y_target)
...
for train_idx, test_idx in cv.split(X_imp, y_bin):
    model = XGBRegressor(**grid.best_params_, random_state=42)
    model.fit(X_imp.iloc[train_idx], y_target.iloc[train_idx])
    y_pred_all[test_idx] = model.predict(X_imp.iloc[test_idx])

final_auc = roc_auc_score(y_bin, y_pred_all)
print(f'Final Model AUC: {final_auc:.5f}')

scaler = MinMaxScaler()
y_pred_all = scaler.fit_transform(y_pred_all.reshape(-1, 1)).flatten()
...
report_md = f"""# 🌊 일광 해수욕장 수질 AI 예측 입수 통제 보고서
...
* **핵심 모델링 기법**: 
  * 연속형 오염 수치를 정확히 추적하는 XGBRegressor 최적화
