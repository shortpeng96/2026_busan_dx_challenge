...
ax.set_title(f'다대포 해수욕장 - 수질 예측 핵심 변수 중요도', color=C_NAVY, weight='bold', fontsize=16)
...
ax.set_title(f'다대포 해수욕장 - 카테고리별 오염 기여도', color=C_NAVY, weight='bold', fontsize=16)
plt.tight_layout()
...
plt.suptitle(f'다대포 해수욕장 이중 기준선 혼동행렬 (Confusion Matrix)', color=C_NAVY, weight='bold', fontsize=16)
plt.tight_layout()
...
ax.set_title(f'다대포 해수욕장 - 정상/오염 데이터별 예측 점수 분포도 (Log Scale)', color=C_NAVY, weight='bold', fontsize=16)
...
baseline_precision = (23 / (23 + 10)) * 100
...
plt.suptitle(f'다대포 해수욕장 - AI 도입 효과 (ROI) 시각화', color=C_NAVY, weight='bold', fontsize=16)
plt.tight_layout()
