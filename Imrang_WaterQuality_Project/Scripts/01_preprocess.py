...
BEACH = 'Imrang'
ECOLI_THRESHOLD = 500
ENTERO_THRESHOLD = 100  # 장구균 기준 (임랑은 장구균 초과가 주요 지표)
...
if 'examinDe' in df_w.columns:
    ...
    # Remove thousands separators before numeric conversion (e.g. "1,724").
    df_w['ecoli'] = pd.to_numeric(df_w['water01'].astype(str).str.replace(',', '', regex=False), errors='coerce')
    # water02 = 장구균(enterococcus)
    df_w['enterococcus'] = pd.to_numeric(df_w['water02'].astype(str).str.replace(',', '', regex=False), errors='coerce') if 'water02' in df_w.columns else np.nan
...
# any_exceed: 대장균 >=500 OR 장구균 >=100 (임랑은 장구균 초과가 지배적)
