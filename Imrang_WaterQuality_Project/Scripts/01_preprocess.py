...
print("[Imrang] Preprocessing data from 10 distinct category CSVs...")
...
# 1. Read all 10 files
df_precip = read_raw_csv('임랑_강수.csv')
df_temp = read_raw_csv('임랑_기온.csv')
df_wind = read_raw_csv('임랑_바람.csv')
df_uv = read_raw_csv('임랑_자외선.csv')
df_sewage = read_raw_csv('임랑_인근_하수_방류량.csv')
df_river = read_raw_csv('임랑_인근_하천_방류량.csv')
df_visitor = read_raw_csv('임랑_방문객.csv')
df_tide = read_raw_csv('임랑_조수.csv')
df_water = read_raw_csv('임랑_수질.csv')
df_temp_water = read_raw_csv('임랑_부이데이터.csv')

# 2. Base Date Frame (Use Water Quality as the base target)
if df_water.empty:
    print("  Error: 임랑_수질.csv is empty or missing. Cannot build target dataset.")
    exit(1)
...
if 'munoseong_discharge_m3_day' in feature_table.columns:
    feature_table['munoseong_discharge_1d_lag'] = feature_table['munoseong_discharge_m3_day'].shift(1)
    feature_table['munoseong_discharge_3d_mean'] = feature_table['munoseong_discharge_m3_day'].rolling(3).mean().shift(1)
...
for c in feature_table.columns:
    ...
    elif '해수욕장' in c and '_1d_lag' not in c: # Handle Imrang/Songjeong buoy names
        feature_table[f"{c}_1d_lag"] = feature_table[c].shift(1)
    elif '송정' in c and '_1d_lag' not in c:
        feature_table[f"{c}_1d_lag"] = feature_table[c].shift(1)
    elif '임랑' in c and '_1d_lag' not in c and c != '임랑해수욕장': # Imrang specific
        feature_table[f"{c}_1d_lag"] = feature_table[c].shift(1)
...
print("[Imrang] Preprocessing complete!")
