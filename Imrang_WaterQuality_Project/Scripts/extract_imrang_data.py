...
BASE = 'c:/Sandbox/2026_busan_dx_challenge/Imrang_WaterQuality_Project'
...
df_om[['time', 'precipitation_sum']].rename(columns={'time':'date', 'precipitation_sum':'precip_daily'}).to_csv(os.path.join(NEW_RAW, '임랑_강수.csv'), index=False)
df_om[['time', 'temperature_2m_mean']].rename(columns={'time':'date', 'temperature_2m_mean':'temp_daily'}).to_csv(os.path.join(NEW_RAW, '임랑_기온.csv'), index=False)
df_om[['time', 'wind_speed_10m_max', 'wind_direction_10m_dominant']].rename(columns={'time':'date', 'wind_speed_10m_max':'wind_max', 'wind_direction_10m_dominant':'wind_dir'}).to_csv(os.path.join(NEW_RAW, '임랑_바람.csv'), index=False)
df_om[['time', 'shortwave_radiation_sum']].rename(columns={'time':'date', 'shortwave_radiation_sum':'solar_radiation_sum'}).to_csv(os.path.join(NEW_RAW, '임랑_자외선.csv'), index=False)
...
# 5. Water Quality (From busan_beach_임랑.csv)
print('Extracting Water Quality...')
f_water = 'C:/Sandbox/Water_Quality/busan_beach_임랑.csv'
try: df_wq = pd.read_csv(f_water, encoding='utf-8-sig')
except: df_wq = pd.read_csv(f_water, encoding='cp949', errors='replace')

# busan_beach_임랑.csv columns: ctprvnNm,gugunNm,beachKoreanNm,ctprvnCode,gugunCode,stnpntCode,examinOdr,examinLcSeCode,coliDetectCn,entrcccsDetectCn,examinYear,examinDe,examinKnd,examinKndKor,stbltAt,examinLcDetail,la,lo,tempbtn,examinReExamCnt
# The examinDe column might be empty for 2014 records. We can try to use it if available.
...
df_grouped[['date', 'examinLcDetail', 'ecoli_max', 'enterococcus_max']].to_csv(os.path.join(NEW_RAW, '임랑_수질.csv'), index=False)

# 6. Sewage
print('Extracting Sewage...')
sewage_dfs = []
for station in ['기장사업소 (기장)', '정관사업단', '기장사업소 (문오성)']:
    if '기장사업소 (기장)' in station: prefix = 'gijang'
    elif '정관' in station: prefix = 'jeonggwan'
    else: prefix = 'munoseong'
    
    f_discharge = os.path.join('C:/Sandbox/Discharge', f'{station} 일일방류량.csv')
...
if sewage_dfs:
    ...
    sewage_df.to_csv(os.path.join(NEW_RAW, '임랑_인근_하수_방류량.csv'), index=False)
...
if tide_records:
    ...
    df_tide_daily.to_csv(os.path.join(NEW_RAW, '임랑_조수.csv'), index=False)
else:
    pd.DataFrame(columns=['date']).to_csv(os.path.join(NEW_RAW, '임랑_조수.csv'), index=False)
...
df_temp_daily.to_csv(os.path.join(NEW_RAW, '임랑_수온.csv'), index=False)

# 9. Empties
print('Creating empties...')
pd.DataFrame(columns=['date']).to_csv(os.path.join(NEW_RAW, '임랑_방문객.csv'), index=False)
pd.DataFrame(columns=['date']).to_csv(os.path.join(NEW_RAW, '임랑_인근_하천_방류량.csv'), index=False)
