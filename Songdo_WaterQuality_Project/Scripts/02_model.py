...
# t_red: 더 엄격한 임계값 — t_yellow 초과 구간에서 FP를 최소화하면서 Recall >= 40% 유지
t_red = t_yellow
best_red_score = -1
for t in np.arange(min(t_yellow + 0.01, 0.99), 1.0, 0.01):
    preds = (y_pred_proba >= t).astype(int)
    fp = ((preds == 1) & (y_bin == 0)).sum()
    recall_t = recall_score(y_bin, preds, zero_division=0)
    # Recall >= 0.35 (35%)를 유지하면서 FP를 최소화
    if recall_t >= 0.35:
        score = recall_t - fp * 0.05  # FP 패널티
        if score > best_red_score:
            best_red_score = score
            t_red = t

# fallback: t_red == t_yellow이면 t_yellow보다 약간 높은 값으로 강제 설정
if abs(t_red - t_yellow) < 0.001:
    for t in np.arange(t_yellow + 0.01, 1.0, 0.01):
        preds = (y_pred_proba >= t).astype(int)
        recall_t = recall_score(y_bin, preds, zero_division=0)
        if recall_t > 0:
            t_red = t
