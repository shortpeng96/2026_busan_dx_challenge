...
def build_master_dataset():
    ...
    f_water = os.path.join(proj_dir, "Data_Processed", "gwangalli_water_samples.csv")
    df_water = pd.read_csv(f_water, encoding='utf-8-sig')
    df_water['examinDe'] = pd.to_datetime(df_water['examinDe'])
    
    # 2. Weather
    f_weather = os.path.join(proj_dir, "Data_Raw", "gwangalli_weather_2014_2026.csv")
    ...
    f_suyeong = os.path.join(proj_dir, "Data_Processed", "suyeong_sewage_daily.csv")
    ...
    f_nambu = os.path.join(proj_dir, "Data_Processed", "nambu_sewage_daily.csv")
    df_nambu = pd.read_csv(f_nambu, encoding='utf-8-sig')
    df_nambu['date'] = pd.to_datetime(df_nambu['date'])
    df_nambu.rename(columns={'sewage_volume': 'nambu_vol'}, inplace=True)
    
    # Calculate Dynamic Yearly Capacity (99th percentile)
    ...
    # Dual-CSO Flags
    # East (Suyeong): Suyeong 1d_lag >= Suyeong Yearly Cap AND 3-day precip >= 5.0
    ...
    # West (Nambu): Nambu 1d_lag >= Nambu Yearly Cap AND 3-day precip >= 5.0
    ...
    out_path = os.path.join(proj_dir, "Data_Processed", "master_dataset_gwangalli.csv")
