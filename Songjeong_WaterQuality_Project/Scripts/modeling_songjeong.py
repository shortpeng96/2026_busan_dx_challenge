...
df_master = pd.read_csv(os.path.join(proj_dir, "Data_Processed", "songjeong_spatial_master.csv"))

df_master['wind_x_distance'] = df_master['wind_speed_1d_lag'] * df_master['distance_to_outfall']
df_master['suyoung_x_distance'] = df_master['수영사업단_vol_3d_sum_lag'] * df_master['distance_to_outfall']
df_master['gijang_x_distance'] = df_master['기장사업소_vol_3d_sum_lag'] * df_master['distance_to_outfall']
...
features = [
    ...
    '수영사업단_vol_1d_lag', '수영사업단_vol_3d_sum_lag', 
    '기장사업소_vol_1d_lag', '기장사업소_vol_3d_sum_lag',
    'suyoung_x_distance', 'gijang_x_distance'
]
...
cv = StratifiedKFold(n_splits=3, shuffle=True, random_state=42) # n_splits=3 because only 6 exceedances
current_features = list(features)
results = []

while len(current_features) > 0:
    aucs = []
    xgb = XGBClassifier(n_estimators=100, learning_rate=0.05, max_depth=3, subsample=0.8, colsample_bytree=0.8, scale_pos_weight=scale_pos, random_state=42)
...
xgb_final = XGBClassifier(n_estimators=100, learning_rate=0.05, max_depth=3, subsample=0.8, colsample_bytree=0.8, scale_pos_weight=scale_pos, random_state=42)
...
plt.title(f'송정 해수욕장 공간/기상 융합 AI (AUC: {best_auc:.3f})')
plt.tight_layout()
plt.savefig(os.path.join(proj_dir, "Results", "feature_importance_Songjeong.png"))

with open(os.path.join(proj_dir, "Results", "results_Songjeong.txt"), "w") as f:
    f.write(f"=== Songjeong Spatial-Wind AI Results ===\n")
